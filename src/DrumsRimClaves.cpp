...
enum LightIds {
 ...
 unsigned int count1  = RIM_sample1_len;
    unsigned int count2  = RIM_sample2_len;
    unsigned int count3  = RIM_sample3_len;
    unsigned int count4  = RIM_sample4_len;
    unsigned int count5  = RIM_sample5_len;
    unsigned int count6  = RIM_sample6_len;
    unsigned int count7  = RIM_sample7_len;
    unsigned int count8  = RIM_sample8_len;

	
	DrumsRimClaves()  {
		config(NUM_PARAMS, NUM_INPUTS, NUM_OUTPUTS, NUM_LIGHTS);
configParam(DrumsRimClaves::SAMPLETYPE, 0.0, 1.0, 0.0, "");
//configParam(DrumsRimClaves::PITCH, -12, 12.0, 0.0, "");
...
void DrumsRimClaves::process(const ProcessArgs &args)
{
	...
	if (sampletype == 1)
    {
                if( count1 < RIM_sample1_len ) {
                    ...
                    outputs[AUDIO_OUTPUT].setVoltage( 5.0 * (float)sample / RIM_sample1_len );
                } else {
    ...
    if (sampletype == 2)
    {
        if( count2 < RIM_sample2_len ) {
            ...
            outputs[AUDIO_OUTPUT].setVoltage( 5.0 * (float)sample / RIM_sample2_len );
        } else {
            outputs[AUDIO_OUTPUT].setVoltage( 0.0 );
        }
    }
    ...
    if (sampletype == 3)
    {
        if( count3 < RIM_sample3_len ) {
            ...
            outputs[AUDIO_OUTPUT].setVoltage( 5.0 * (float)sample / RIM_sample3_len );
        } else {
            outputs[AUDIO_OUTPUT].setVoltage( 0.0 );
        }
    }
    ...
    if (sampletype == 4)
    {
        if( count4 < RIM_sample4_len ) {
            ...
            outputs[AUDIO_OUTPUT].setVoltage( 5.0 * (float)sample / RIM_sample4_len );
        } else {
            outputs[AUDIO_OUTPUT].setVoltage( 0.0 );
        }
    }
    ...
    if (sampletype == 5)
    {
        if( count5 < RIM_sample5_len ) {
            ...
            outputs[AUDIO_OUTPUT].setVoltage( 5.0 * (float)sample / RIM_sample5_len );
        } else {
            outputs[AUDIO_OUTPUT].setVoltage( 0.0 );
        }
    }
    ...
    if (sampletype == 6)
    {
        if( count6 < RIM_sample6_len ) {
            ...
            outputs[AUDIO_OUTPUT].setVoltage( 5.0 * (float)sample / RIM_sample6_len );
        } else {
            outputs[AUDIO_OUTPUT].setVoltage( 0.0 );
        }
    }
    ...
    if (sampletype == 7)
    {
        if( count7 < RIM_sample7_len ) {
            ...
            outputs[AUDIO_OUTPUT].setVoltage( 5.0 * (float)sample / RIM_sample7_len );
        } else {
            outputs[AUDIO_OUTPUT].setVoltage( 0.0 );
        }
    }
    ...
    if (sampletype == 8)
    {
        if( count8 < RIM_sample8_len ) {
            ...
            outputs[AUDIO_OUTPUT].setVoltage( 5.0 * (float)sample / RIM_sample8_len );
        } else {
            outputs[AUDIO_OUTPUT].setVoltage( 0.0 );
        }
    }
...
struct DrumsRimClavesWidget : ModuleWidget {
    ...
    //addParam(createParam<Davies1900hBlackKnob>(Vec(30, 80), module, DrumsRimClaves::PITCH));
